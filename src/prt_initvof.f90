module prt_mod_initvof
  ...
  contains
  !
  subroutine initvof(n,unew,vnew,wnew,uphase,vphase,wphase)
    implicit none
    integer, dimension(3), intent(in) :: n
    real(rp), dimension(1-nh_wide:,1-nh_wide:,1-nh_wide:), intent(in) :: unew,vnew,wnew
    real(rp), dimension(1-nh_wide:,1-nh_wide:,1-nh_wide:), intent(inout) :: uphase,vphase,wphase
    type pneighbor
       real(rp) :: x,y,z
    end type pneighbor
    ...
    real(rp) :: radin,radin2,dist2c,dist2x,dist2y,dist2z
    real(rp) :: dx,dy,dz
            ...
            do k=klow,khigh
              do j=jlow,jhigh
                do i=ilow,ihigh
                  ...
                  !
!                  if (alpha_eulx>0.5_rp) then
!                    uphase(i,j,k)=ul
!                  end if
!                  if (alpha_euly>0.5_rp) then
!                    vphase(i,j,k)=vl
!                  end if
!                  if (alpha_eulz>0.5_rp) then
!                    wphase(i,j,k)=wl
!                  end if
                  uphase(i,j,k)=(1._rp-alpha_eulx)*unew(i,j,k)+alpha_eulx*ul
                  vphase(i,j,k)=(1._rp-alpha_euly)*vnew(i,j,k)+alpha_euly*vl
                  wphase(i,j,k)=(1._rp-alpha_eulz)*wnew(i,j,k)+alpha_eulz*wl
                  !
                enddo ! do i=
                !
              enddo ! do j=
